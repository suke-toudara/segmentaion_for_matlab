...
imdsTrain_sub = imageDatastore(imgDir);
imdsTrain = transform(imdsTrain_sub,@augmentationFcn);
...
%学習データ作成
dataFolder = fullfile(pwd,'data'); 
imgDir = fullfile(dataFolder,'val');
imdsVal_sub = imageDatastore(imgDir);
imdsVal = transform(imdsVal_sub,@augmentationFcn);


%学習用ラベルデータ作成
labelIDs = PixelLabelIDs(); %クラスに対応するRGBカラーを指定
dataFolder = fullfile(pwd,'data'); 
imgDir = fullfile(dataFolder,'val_label');
